...
| `transit.py` | `GET https://external.transitapp.com/v4/public/nearby_stops?lat=&lon=` (Transit App developer API; key at `~/.config/osm/transit_api.json`) | `~/.config/osm/transit_cache/` | per-endpoint (1 min real-time → 7 days metadata) | `cross_check_bus_stop_findings()`: second authoritative source for `misplaced_bus_stops` — suppresses a finding when Transit knows a stop within 50 m. One call per flagged stop; monthly-quota + rate-limit aware (see [`transit-quota.md`](transit-quota.md)) |
