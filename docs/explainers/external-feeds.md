...
| `transit.py` | Transit App developer API (`https://external.transitapp.com/v4/public/{nearby_stops,available_networks,alerts_for_networks,...}`; key at `~/.config/osm/transit_api.json`) | `~/.config/osm/transit_cache/` | per-endpoint (1 min real-time → 7 days metadata) | `cross_check_bus_stop_findings()`: second authoritative source for `misplaced_bus_stops` — suppresses a finding when Transit knows a stop within 50 m (one call per flagged stop). `fetch_sorta_alerts()` / `resolve_sorta_network_id()`: SORTA service alerts for `osm transit-alerts`. Monthly-quota + rate-limit aware (see [`transit-quota.md`](transit-quota.md)) |
| `gtfs_rt.py` | SORTA GTFS-Realtime (Trapeze): `https://tmgtfsprd.sorttrpcloud.com/TMGTFSRealTimeWebService/{vehicle/vehiclepositions,tripupdate/tripupdates}.pb` — protobuf, no API key, no quota | `~/.config/osm/gtfs_rt_cache/{feed}.pb` | 30 s | `vehicle_positions()` / `trip_updates()`: low-latency real-time source for `osm gtfs-rt` and `GET /api/gtfs-rt/:feed`. Parsed with `gtfs-realtime-bindings`; the unusual `application/protocol-buffer` content-type and the static-zip 301 are handled server-side. Complements `transit.py` (which stays primary for normalized alerts + network discovery) |
